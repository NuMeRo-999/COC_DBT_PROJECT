...
SELECT
    ...
    (dc.donations_received - COALESCE(LAG(dc.donations_received) OVER (PARTITION BY dc.player_id ORDER BY dc.date_key), 0)) as net_donations
FROM daily_changes dc
WHERE dc.date_key >= DATEADD('day', -90, CURRENT_DATE()) -- Últimos 90 días